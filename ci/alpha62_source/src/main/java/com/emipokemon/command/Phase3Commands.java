                ...
                .then(literal("give")
                        ...
                        .then(literal("machine")
                                ...
                                .then(literal("standard")
                                        .executes(context -> give(context.getSource(), ModRegistries.STANDARD_GACHA_MACHINE.asItem(), 1, "maquina gacha estandar"))
                                        .then(argument("amount", IntegerArgumentType.integer(1, 64))
                                                .executes(context -> give(
                                                        context.getSource(),
                                                        ModRegistries.STANDARD_GACHA_MACHINE.asItem(),
                                                        IntegerArgumentType.getInteger(context, "amount"),
                                                        "maquina gacha estandar"
                                                ))))
                                .then(literal("emi")
                                        .executes(context -> give(context.getSource(), ModRegistries.EMI_GACHA_MACHINE.asItem(), 1, "maquina gacha de Emi"))
                                        .then(argument("amount", IntegerArgumentType.integer(1, 64))
                                                .executes(context -> give(
                                                        context.getSource(),
                                                        ModRegistries.EMI_GACHA_MACHINE.asItem(),
                                                        IntegerArgumentType.getInteger(context, "amount"),
                                                        "maquina gacha de Emi"
                                                )))))
                        .then(literal("ticket")
                                .then(literal("standard")
