...
public final class BannerDefinition {
    ...
    /** Zero means no lower/upper time bound. Stored as UTC epoch milliseconds. */
    public long startsAtEpochMillis = 0L;
    public long endsAtEpochMillis = 0L;
    ...
    public void normalize() {
        ...
        startsAtEpochMillis = Math.max(0L, startsAtEpochMillis);
        endsAtEpochMillis = Math.max(0L, endsAtEpochMillis);
        if (startsAtEpochMillis > 0L && endsAtEpochMillis > 0L && endsAtEpochMillis <= startsAtEpochMillis) {
            endsAtEpochMillis = 0L;
        }
        rotationPriority = Math.clamp(rotationPriority, -1_000, 1_000);
    ...
    public boolean activeAt(long epochMillis) {
        return enabled
                && (startsAtEpochMillis == 0L || epochMillis >= startsAtEpochMillis)
                && (endsAtEpochMillis == 0L || epochMillis < endsAtEpochMillis);
    }
