...
# Advance all inherited alpha.39 version assertions without weakening them.
for rel in [
 ...
 'src/test/java/com/emipokemon/casino/CasinoMaterialScaleRegressionTest.java',
    ...
    p=root/rel
    if p.exists():
        s=p.read_text().replace('0.4.0-alpha.39','0.4.0-alpha.40')
        s=s.replace('alpha39VersionIsConsistentInSource','alpha40VersionIsConsistentInSource')
        s=s.replace('alpha39VersionIsConsistent','alpha40VersionIsConsistent')
        s=s.replace('alpha39VerticalCabinetsAreApproximatelyTwoBlocksTall','alpha40VerticalCabinetsAreEssentiallyTwoBlocksTall')
        s=s.replace('alpha39TablesStayAtFurnitureHeight','alpha40TablesStayAtFurnitureHeight')
        s=s.replace('alpha39RendererSupportsTallGeometry','alpha40RendererSupportsTallGeometry')
        s=s.replace('alpha39TexturesContainMaterialVariation','alpha40TexturesContainMaterialVariation')
        # alpha.40 deliberately tightens the cabinet target from merely tall to almost exactly two blocks.
        s=s.replace('assertTrue(h >= 28.0, id+" should read as a tall arcade cabinet: "+h);','assertTrue(h >= 31.8, id+" should reach essentially two blocks: "+h);')
        p.write_text(s)

p=root/'src/test/java/com/emipokemon/casino/CasinoConstructionDetailRegressionTest.java'
...
class CasinoConstructionDetailRegressionTest {
    ...
    @Test
    void alpha40VersionIsConsistent() throws Exception {
        assertTrue(Files.readString(Path.of("gradle.properties")).contains("mod_version=0.4.0-alpha.40"));
        assertTrue(Files.readString(Path.of("src/main/java/com/emipokemon/Emipokemon.java")).contains("0.4.0-alpha.40"));
    }
}
''')
