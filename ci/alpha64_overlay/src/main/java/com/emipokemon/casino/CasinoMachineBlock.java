...
public final class CasinoMachineBlock extends BlockWithEntity {
    ...
    private static final VoxelShape CLAW_SHAPE = Block.createCuboidShape(1, 0, 1, 15, 38, 15);
    private static final VoxelShape TABLE_SHAPE = Block.createCuboidShape(0, 0, 0, 16, 12, 16);
    private static final VoxelShape POKEMON_FLIP_SHAPE = Block.createCuboidShape(0, 0, 0, 16, 14, 16);
    ...
    protected VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        ...
        if (type == CasinoGameType.CLAW) return CLAW_SHAPE;
        if (type == CasinoGameType.POKEMON_FLIP) return POKEMON_FLIP_SHAPE;
        return type == CasinoGameType.ROULETTE || type == CasinoGameType.POKER
                || type == CasinoGameType.BLACKJACK || type == CasinoGameType.DICE
                || type == CasinoGameType.POKEMON_FLIP ? TABLE_SHAPE : MACHINE_SHAPE;
    }
